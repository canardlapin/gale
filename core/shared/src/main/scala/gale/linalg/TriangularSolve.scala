...
/** Standalone triangular solves `L x = b` (lower) and `U x = b` (upper) for a dense triangular matrix `A`.
  *
  * `A` is read as-is: only the triangle named by the entry point participates (the opposite triangle is ignored, not
  * required to be zero), and the diagonal is used directly — a zero on the diagonal makes the system singular and
  * yields `Left(`[[LinAlgError.SingularMatrix]]`)` at that pivot. `A` may be any dense view (strided, transposed, or a
  * submatrix); the solve honours its strides.
  *
  * Both methods share the single strided substitution kernel `gale.kernel.DoubleKernels.dtrsv`, which also backs the LU
  * / Cholesky / QR back-substitutions, so there is one triangular-solve implementation.
  */
object TriangularSolve:
  ...
  private def solve(A: DMat, b: DVec, lower: Boolean): Either[LinAlgError, DVec] =
    val n = A.rows
    if A.cols != n then Left(LinAlgError.NonSquareMatrix(A.shape))
