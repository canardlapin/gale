...
/** An '''optional, zero-cost''' compile-time-dimension layer over the runtime-sized `DVec`/`DMat` (PRD "Optional Sized
  * Layer"). [[SVec]] and [[SMat]] are opaque wrappers carrying the length / shape as Scala 3 literal-integer type
  * parameters, so the core operations (`+`, `-`, scale, `dot`, matrix-vector, matrix-matrix, transpose) are
  * '''shape-checked at compile time''' when the dimensions are statically known, while lowering to the exact same
  * `DVec`/`DMat` kernels.
  *
  * The layer allocates nothing (an opaque alias of `DVec`/`DMat`), is opt-in (nothing in `gale.linalg` or the rest of
  * core references it), and lowers losslessly and '''zero-copy''' through [[SVec.toDVec]] / [[SMat.toDMat]] (the
  * identity on the wrapped value). Raising a runtime value is safe: [[SVec.fromDVec]] / [[SMat.fromDMat]] return `Left`
  * on a shape mismatch; the trusted `unsafe` constructors are `private[sized]`.
  *
  * '''Naming.''' The PRD sketches `Matrix.Sized[A, R, C]` / `Vec.Sized[A, N]` inside `object Matrix` / `object Vec`;
  * those objects live in `gale.linalg`. This layer ships in its own package `gale.sized` as [[SMat]]/[[SVec]]
  * (Double-specialized, matching gale's `DVec`/`DMat` op tier) so it stays fully additive and leaks nothing into the
  * core API.
  */
opaque type SVec[N <: Int] = DVec

/** A compile-time-sized matrix: `R` rows, `C` columns, over `DMat`. See [[SVec]] for the layer's design (optional,
  * zero-cost, opt-in).
  */
...
object SVec:
  ...
  /** Adopt a runtime `DVec` as a statically-sized `SVec[N]`, or `Left` if its length is not `N` (zero-copy on success).
    */
  ...
  /** Build an `SVec[N]` from exactly `N` values (`N` inferred from the expected type). Throws
    * `IllegalArgumentException` on an arity mismatch — a programming error, not runtime data — so the size stays a
    * compile-time fact.
    */
...
object SMat:
  ...
  /** Adopt a runtime `DMat` as a statically-sized `SMat[R, C]`, or `Left` on a shape mismatch (zero-copy on success).
    */
  ...
  /** Build an `SMat[R, C]` from exactly `R·C` '''row-major''' values (`R`, `C` inferred from the expected type). Throws
  ...
  extension [R <: Int, C <: Int](m: SMat[R, C])
    ...
    /** Matrix-matrix product (shape-checked: the right factor's rows must be `C`). Forwards the ambient `given Backend`
      * to the underlying `DMat.*` so an imported accelerating backend routes through the sized layer too (it would
      * otherwise capture `Backend.pure`).
      */
  ...
  extension (m: SMat[2, 2])
    ...
    /** Inverse of a `2×2`, or `Left(SingularMatrix)` when the determinant is zero / non-finite (unrolled adjugate).
      *
      * '''Conditioning caveat.''' The singularity test is '''exact''' (`det == 0` or non-finite): a merely
      * '''near'''-singular matrix returns `Right` with an ill-conditioned, large-magnitude inverse in which roundoff is
      * amplified by `1/det`. This closed-form kernel does no pivoting or condition estimation — for a numerically
      * robust solve of an ill-conditioned system, lower to `DMat` and use gale's LU/QR (`toDMat.solve` /
      * `toDMat.leastSquares`).
      */
  ...
  extension (m: SMat[3, 3])
    ...
    /** Inverse of a `3×3`, or `Left(SingularMatrix)` when singular (unrolled adjugate transpose divided by the
      * determinant).
      *
      * '''Conditioning caveat.''' As with the `2×2` inverse overload, the singularity test is '''exact''' (`det == 0`
      * or non-finite); a near-singular matrix yields a `Right` with a large-magnitude, roundoff-amplified inverse. For
      * ill-conditioned data, lower to `DMat` and solve via gale's LU/QR.
      */
