...
/** The result of a real '''symmetric''' eigenproblem — ordinary `A V = V diag(λ)` or symmetric-definite generalized
  * `A V = B V diag(λ)` — complex-free by construction (fixed constraint 3 of `docs/spectral-parity.md`).
  *
  * Eigenvalues are returned '''ascending-algebraic always''', independent of the selection order (§ 1): the selection
  * chooses membership, this layout is fixed. `eigenvectors` holds the orthonormal eigenvectors as columns, aligned with
  * `eigenvalues`; it is empty (zero columns) when only values were requested. Ordinary eigenvectors are
  * Euclidean-orthonormal. Generalized eigenvectors are `B`-orthonormal; in that case
...
/** The result of a '''nonsymmetric''' eigenproblem, whose real input may have complex eigenvalues in conjugate pairs (§
  * 2, § 7).
  *
  * Storage is real throughout: eigenvalues as structure-of-arrays real/imaginary parts (`re`, `im`), right eigenvectors
  * as a real `DMat` in LAPACK '''real-Schur packed''' convention, and optional left eigenvectors in the same packing.
  * That packing is an implementation detail and is '''never''' exposed — the storage fields are `private[gale]` and
  * callers read the spectrum only through the typed accessors below.
  *
  * '''Packing invariant.''' A real eigenvalue owns one column (its real eigenvector, zero imaginary part). A
  * complex-conjugate pair `λ = a ± b·i` (`b > 0`) owns two '''adjacent''' columns `v_re, v_im`, with the
  * '''positive-imaginary member first''': the eigenvalue `a + b·i` at index `j` has eigenvector `v_re + i·v_im`
  * (columns `j`, `j+1`), and its conjugate `a − b·i` at index `j+1` has eigenvector `v_re − i·v_im`. [[eigenvector]]
  * and [[leftEigenvector]] decode this, so no caller ever sees the shared columns.
  */
final class NonsymmetricEigenDecomposition private[spectral] (
  ...
  /** True when the `i`-th eigenvalue is real (zero imaginary part), so it owns a single real eigenvector column rather
    * than a shared pair of columns. An out-of-range `i` throws [[gale.linalg.LinAlgError.IndexOutOfBounds]].
    */
  def isRealPair(i: Int): Boolean =
    checkIndex(i)
    im(i) == 0.0

  /** The `i`-th '''right''' eigenvector as `(realPart, imaginaryPart)`, decoded from the real-Schur packing. For a real
    * eigenvalue the imaginary part is zero; for the two members of a conjugate pair the imaginary parts are negatives
    * of each other. Throws [[gale.linalg.LinAlgError.UnsupportedOperation]] if vectors were not computed, or
    * [[gale.linalg.LinAlgError.IndexOutOfBounds]] if `i` is out of range.
    */
  def eigenvector(i: Int): (DVec, DVec) =
    checkIndex(i)
    decode(rightVectorsPacked, "right eigenvectors", i)

  /** The `i`-th '''left''' eigenvector as `(realPart, imaginaryPart)`, decoded like [[eigenvector]]. Throws
    * [[gale.linalg.LinAlgError.UnsupportedOperation]] if left vectors were not computed, or
    * [[gale.linalg.LinAlgError.IndexOutOfBounds]] if `i` is out of range.
    */
  ...
  /** Decode column(s) of a packed real-Schur eigenvector matrix at index `i` into explicit `(realPart, imaginaryPart)`
    * vectors. The imaginary part is a fresh vector; the real part is a copy, so no view into the internal packing
    * escapes. Vector length is the ambient dimension `rightVectorsPacked.rows`, which for a partial result exceeds
    * [[size]].
    */
