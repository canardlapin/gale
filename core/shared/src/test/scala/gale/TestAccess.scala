...
/** Controlled access to `private[gale]` storage for the few suites that legitimately need to build strided/aliased
  * layouts or drive a kernel directly. Centralising it here keeps the rest of the test suite on the public API, so any
  * test poking at raw storage does so through this one auditable door.
  */
object TestAccess:
  ...
  /** A platform array of `length` cells all set to `value`. Useful for building a backing buffer whose untouched cells
    * act as sentinels.
    */
  ...
  /** A stride-`stride` view of `x`'s values scattered across a fresh backing buffer (the intervening cells are zero).
    * Exercises strided kernels.
    */
  ...
  /** Snapshot of the entire backing storage behind `v` (not just the view), so a test can assert that a strided write
    * left neighbouring cells intact.
    */
  ...
  /** The reusable scratch buffer behind a workspace, for asserting genuine reuse across factorizations.
    */
  ...
  /** Build a CSR directly from raw row-pointer/column-index/value arrays, with no canonicalization. Lets a suite
    * construct deliberately non-canonical input (unsorted columns, duplicate columns, explicit zeros) that the public
    * API never produces, to exercise `canonicalize`/`sortedIndices`/`prune`.
    */
  ...
  /** Build a CSC directly from raw column-pointer/row-index/value arrays, with no canonicalization. The CSC dual of
    * [[csr]].
    */
  ...
  /** Build a COO directly from raw triples, letting a suite include duplicate coordinates that the public builder
    * always sums away. `canonical` sets the advertised flag without validation.
    */
  ...
  /** A mutable vector view onto `base`'s backing storage, offset into it. Used to write a kernel result into the middle
    * of a larger buffer.
    */
  ...
  /** Overwrite every backing element of `m` with NaN, to prove that kernels claiming `beta == 0` assign their
    * destination rather than read-and-scale it.
    */
  ...
  /** Drive the raw dense matrix-matrix kernel `C := A * B` with `beta == 0`, exercising whatever storage layouts `a`,
    * `b`, and `c` present.
    */
