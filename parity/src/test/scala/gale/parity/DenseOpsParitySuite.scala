...
/** Dense BLAS-level parity: gale's `DMat`/`DVec` products and combinations versus
  ...
  *
  * These are same-arithmetic operations (both ultimately a sum of products), so they must agree to a tight elementwise
  * relative tolerance; the only expected difference is summation order (gale's straight loops vs Breeze's netlib BLAS),
